  ...
  return (
      <div className="my-8 relative">
        ...
        {mainData[personNumber].messagesReceives.map((message) => {
          // Normalize both to string in case one is number and the other string
          const sender = secondData.find(
              (tutor) => String(tutor.id) === String(message.messageSender)
          );
          ...
          const senderName =
              (sender?.user?.first_name || sender?.user?.last_name)
                  ? `${sender?.user?.first_name ?? ""} ${sender?.user?.last_name ?? ""}`.trim()
                  : "Unknown Sender";

...
// ❌ remove: import { Tutor } from "@/model/tutorType";
...
type DashboardStudentMessagesSentProps = {
  mainData: Student[];
  secondData: TutorLite[]; // ✅ matches your mock
};
...
const DashboardStudentMessagesSent = ({
  ...
  const student = mainData[personNumber]; // optional guard if needed

  return (
      <div className="my-8">
        <h2 className="text-2xl font-bold text-[#45444A] m-4">My Sent Messages</h2>

        {student.messagesSent.map((message) => {
          // message.messageReceive looks like the recipient tutorId
          const recipient = secondData.find(
                ...
                >
                  <div className="flex justify-between items-center">
                    <p className="font-bold">
                      {recipient
                          ? `${recipient.tutorFirstName} ${recipient.tutorLastName}`
                          : "Unknown Recipient"}
                    </p>

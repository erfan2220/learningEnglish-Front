...
interface TutorCartProps { tutorData: Tutor; }

const TutorCart = ({ tutorData }: TutorCartProps) => {
  const VideoPlayer = dynamic(() => import("../VideoPlayer/VideoPlayer"), { ssr: false });

  const fullName = `${tutorData.user.first_name} ${tutorData.user.last_name}`.trim();
  const mp4Src = tutorData.intro_video_file && tutorData.intro_video_file.trim() !== "" ? tutorData.intro_video_file : null;
          ...
          <div className="flex flex-wrap gap-4">
            {tutorData.languages_spoken.map((lang) => (
                <div key={`${tutorData.id}-${lang}`}>
                  <Country countryName={lang} width={20} textSize="14px" fontWeight="bold" />
                </div>
            ))}
          </div>

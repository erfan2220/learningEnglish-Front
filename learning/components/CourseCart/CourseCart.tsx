import { CourseData } from "@/model/types";
import React from "react";
import Image from "next/image";

const CourseCart = ({ course }: { course: CourseData }) => {
  return (
    <div className="relative pb-4 px-4 border-2 border-[#D2D2D2] bg-white/70 rounded-2xl shadow-md hover:shadow-2xl hover:scale-105  transition-all duration-300">
      ...
      <div className="flex items-center gap-2 my-2">
        ...
        <span className="font-bold text-[#45444A]">{course.tutor}</span>
      </div>

      <div className="flex text-[#45444A] text-sm font-semibold gap-2 my-2">
        speaks:{" "}
        {course.speaks.map((Language, index) => (
      ...
      <p className="mt-4 text-lg text-[#45444A] font-bold">{course.price}</p>
    </div>
  );
};
